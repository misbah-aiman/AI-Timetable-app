...
const STEPS = ['sleep', 'study', 'hobbies', 'screen', 'classes'] as const;
type MainStep = typeof STEPS[number];
type Step = MainStep | 'generating';
type ScanState = 'idle' | 'scanning' | 'done' | 'error';

const STEP_LABELS: Record<MainStep, { title: string; subtitle: string }> = {
  sleep:   { title: 'Sleep Schedule',     subtitle: 'Set your bedtime and wake-up time.' },
  study:   { title: 'Study Goal',         subtitle: 'How many hours will you study each day?' },
  hobbies: { title: 'Your Hobbies',       subtitle: 'Select any you enjoy — AI will fit them in.' },
  screen:  { title: 'Screen Time Limit',  subtitle: 'How much casual scrolling per day is your goal?' },
  classes: { title: 'Classes & Schedule', subtitle: 'Upload a timetable photo or add classes manually.' },
};
...
export const OnboardingWizard = () => {
  ...
  const validateStep = (): boolean => {
    if (step === 'sleep') {
      ...
      const hours = computeSleepHours(data.sleepTime, data.wakeTime);
      if (hours < 1) {
  ...
  const validateAndAddClass = () => {
    ...
    if (newClass.endTime <= newClass.startTime) {
      setClassError('End time must be after the start time.');
      return;
    }
    setClassError('');
  ...
  const handleImageSelect = (file: File) => {
    ...
    if (!file.type.startsWith('image/')) {
      setScanError('Only image files are supported (JPG, PNG, WEBP).');
      return;
    }
    if (file.size > 8 * 1024 * 1024) {
      setScanError('Image must be under 8 MB. Please choose a smaller file.');
      return;
    }
    setScanState('idle');
    setImageFile(file);
  ...
  const handleScan = async () => {
    ...
    try {
      ...
      // Merge — skip exact duplicates (same name + day + startTime)
      const merged = [...data.classes];
      let added = 0;
      for (const cls of extracted) {
        const isDup = data.classes.some(
  ...
  const computedSleep = computeSleepHours(data.sleepTime, data.wakeTime);

  // ── Render ───────────────────────────────────────────────

  return (
    <div className="min-h-screen bg-surface-50 dark:bg-[#16141f] flex items-center justify-center p-4">
      <div className="w-full max-w-lg">
              ...
              return (
                <div key={s} className="flex items-center gap-1.5">
                  <div className={`flex items-center justify-center rounded-full transition-all duration-300 ${
                      ...
                      ? 'w-7 h-7 bg-primary-500 text-white'
                      : active
                      ? 'w-7 h-7 bg-primary-500 ring-4 ring-primary-100 dark:ring-primary-900/40 text-white'
                      : 'w-6 h-6 bg-white dark:bg-[#1e1b2e] border-2 border-gray-200 dark:border-gray-700'
                  }`}>
                    {done   && <CheckCircle size={14} />}
                    {active && <span className="text-xs font-bold">{i + 1}</span>}
                  </div>
                  {i < STEPS.length - 1 && (
                    <div className={`h-px w-7 transition-colors duration-300 ${
                      done ? 'bg-primary-400' : 'bg-gray-200 dark:bg-gray-700'
                    }`} />
                  )}
                </div>
              );
        ...
        {isMainStep && (
          <div className="text-center mb-4">
            <p className="text-sm font-semibold text-primary-500">{STEP_LABELS[step as MainStep].title}</p>
          </div>
        )}
        ...
        <Card>
          ...
          {step === 'sleep' && (
            <div className="space-y-5">
              ...
              <div>
                ...
                <input
                  type="range" min={4} max={12} value={data.sleepHours}
          ...
          {step === 'study' && (
            <div className="space-y-5">
              ...
              {/* Large value display */}
              <div className="flex items-center justify-center py-6 bg-surface-50 dark:bg-[#16141f] rounded-2xl">
                <div className="text-center">
                  <p className="text-7xl font-bold text-primary-500 leading-none tabular-nums">
                    {data.studyGoalHours}
                  </p>
              ...
              <div>
                <input
                  type="range" min={1} max={12} value={data.studyGoalHours}
          ...
          {step === 'screen' && (
            <div className="space-y-5">
              ...
              <div className="flex items-center justify-center py-6 bg-surface-50 dark:bg-[#16141f] rounded-2xl">
                <div className="text-center">
                  <p className="text-7xl font-bold text-orange-500 leading-none tabular-nums">
                    {data.screenTimeLimitHours}
                  </p>
              ...
              <div>
                <input
                  type="range" min={0} max={8} value={data.screenTimeLimitHours}
          ...
          {step === 'classes' && (
            <div className="space-y-5">
              ...
              {!imagePreview ? (
                ...
                <div className="space-y-3">
                  ...
                  <div className="relative rounded-2xl overflow-hidden border border-gray-200 dark:border-gray-700">
                    ...
                    <button
                      onClick={clearImage}
                      className="absolute top-2 right-2 p-1.5 rounded-xl bg-black/50 hover:bg-black/70 text-white transition-colors"
                  ...
                  {scanState === 'error' && (
                    <div className="px-4 py-3 bg-red-50 dark:bg-red-900/20 rounded-2xl">
                      ...
                      <button
                        onClick={handleScan}
                        className="text-xs text-primary-500 hover:underline mt-1.5 flex items-center gap-1"
                      >
              ...
              {/* File validation error (shown before image is selected) */}
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-3 gap-2">
                  <div>
                    ...
                    <select
                      value={newClass.day}
                      onChange={e => setNewClass(c => ({ ...c, day: e.target.value }))}
                ...
                <Button onClick={validateAndAddClass} variant="secondary" className="w-full">
                  + Add Class
                </Button>
              </div>
              ...
              {data.classes.length > 0 && (
                <div className="space-y-2 max-h-44 overflow-y-auto pr-1">
                  ...
                  {data.classes.map((c, i) => (
                    <div
                      key={i}
                      className="flex items-center gap-2 px-3 py-2.5 bg-surface-100 dark:bg-[#16141f] rounded-2xl"
                    >
